...
} from './rich-text-selection-management.service';
...
import {
  UrlModalResult
} from '../url-modal/url-modal-result';

/**
 * @internal
 */
@Injectable()
export class SkyRichTextEditorManagementService {
  private editors: { [key: string]: EditorSetting } = {};

  constructor(
    private windowService: SkyAppWindowRef,
    private selectionService: SkyRichTextSelectionManagementService
  ) {}

  public addEditor(id: string, element: HTMLIFrameElement, styleState: SkyuxRichTextEditorStyleState, placeholder?: string): void {
    if (!(id in this.editors)) {
      this.editors[id] = this.createObservers(id, element);
  ...
  public removeEditor(id: string): void {
    if (id in this.editors) {
      SkyRichTextEditorManagementService.removeObservers(this.editors[id]);
  ...
  public setFontSize(id: string, fontSize: number) {
    ...
    SkyRichTextEditorManagementService.cleanUpBlankStyleTags(doc);
  ...
  public getStyleState(id: string): Partial<SkyuxRichTextEditorStyleState> {
    ...
    if (this.editorSelected(documentEl)) {
      return {
        backColor: SkyRichTextEditorManagementService.getColor(documentEl, 'BackColor'),
        fontColor: SkyRichTextEditorManagementService.getColor(documentEl, 'ForeColor'),
  ...
  public focusEditor(id: string): void {
    if (id in this.editors) {
      ...
      this.editors[id].elementRef.focus();
      editor.focus();
  ...
  public getSelectedAnchorTag(editorId: string) {
    const selectedEl = this.getCurrentSelectionParentElement(editorId);

    return SkyRichTextEditorManagementService.getParent(selectedEl, 'a') as HTMLAnchorElement;
  }
  ...
  private getWindowEl(id: string): Window {
    return this.editors[id].elementRef.contentWindow;
  }
  ...
  private getDocumentEl(id: string): Document {
    ...
    if (this.editors[id].elementRef.contentWindow) {
      return this.editors[id].elementRef.contentWindow.document;
    }
    /* istanbul ignore next */
    return this.editors[id].elementRef.contentDocument;
  }

  private getFontSize(id: string) {
    /* istanbul ignore next */
  ...
  private static removeObservers(setting: EditorSetting): void {
    /* istanbul ignore next */
    const documentEl = setting.elementRef.contentWindow ?
       setting.elementRef.contentWindow.document :
       setting.elementRef.contentDocument;
  ...
  private static getColor(documentEl: Document, selector: string) {
  ...
  private static cleanUpBlankStyleTags(doc: Document) {
